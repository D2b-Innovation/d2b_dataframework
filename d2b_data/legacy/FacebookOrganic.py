...
class Facebook:
    def __init__(
        self,
        user_token: str,
        cache_path: str | Path = "debug_tokens.json",
        verbose: bool = False,
    ):
        ...
        self.logger = Verbose(
            active=verbose, alerts_enabled=False, workflow_name="Facebook"
        )
        ...
        if self.cache_path.exists():
            ...
            self._log(
                "__init__", "No cache found. Call .authenticate() to generate the file."
            )
    ...
    def authenticate(self) -> None:
        if self.cache_path.exists():
            self._log(
                "authenticate",
                f"Cache already exists at '{self.cache_path}'. Skipping authentication.",
            )
            return
        ...
        res = requests.get(
            f"{self.BASE_URL}/me/permissions", params={"access_token": self.user_token}
        )
        debug_data["user_permissions"] = res.json().get("data", [])

        self._log("authenticate", "Fetching accessible pages...")
        res = requests.get(
            f"{self.BASE_URL}/me/accounts", params={"access_token": self.user_token}
        )
        pages_data = res.json()
        ...
        self.cache_path.write_text(
            json.dumps(debug_data, indent=4, ensure_ascii=False), encoding="utf-8"
        )
        self.pages = debug_data["pages"]
        self._log(
            "authenticate", f"{len(self.pages)} pages saved to '{self.cache_path}'."
        )
    ...
    def list_accounts(self) -> pd.DataFrame:
        if not self.pages:
            self._log(
                "list_accounts", "No pages available. Call .authenticate() first."
            )
            return pd.DataFrame()
    ...
    def get_posts(self, account_id: str, limit: int = 100) -> list[dict]:
        ...
        self._log(
            "get_posts",
            f"Downloading up to {limit} posts from page ID '{account_id}'...",
        )
        ...
        for endpoint in ["published_posts", "feed"]:
            ...
            while url and len(posts) < limit:
                ...
                posts.extend(batch)
                self._log(
                    "get_posts", f"  → {len(posts)} posts acumulados via '{endpoint}'"
                )
            ...
            if posts:
                self._log(
                    "get_posts", f"{len(posts[:limit])} posts totales via '{endpoint}'."
                )
                return posts[:limit]
    ...
    def get_posts_summary(self, account_id: str, limit: int = 100) -> pd.DataFrame:
        ...
        for post in posts:
            ...
            self._log(
                "kpis",
                f"  [{row['fecha'][:10]}] 👍{row['reacciones']} 💬{row['comentarios']} 🔁{row['compartidos']} | {row['mensaje'][:50] or '(sin texto)'}",
            )
    ...
    def get_current_status(self, account_id: str) -> pd.DataFrame:
        ...
        params = {"access_token": page.get("page_token"), "fields": fields}
    ...
    def query_stat(
        ...
        while current_since <= dt_until:
            ...
            self._log(
                "query_stat",
                f"req: {req} ({current_since.strftime('%Y-%m-%d')} to {current_until.strftime('%Y-%m-%d')})",
            )
